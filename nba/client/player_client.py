...
class PlayerClient:
    ...
    def __init__(self):
        ...
        print('NBA Players fetched: {count}'.format(count=len(self.players)))
    ...
    def get_players_game_log(self, player_arr, season):
        game_logs = pd.DataFrame()

        for player in player_arr:
            df = self.get_player_game_log(player, season)
            game_logs = pd.concat([game_logs, df], ignore_index=True)
            print('{player} game logs fetched'.format(player=player))

        return game_logs

    def get_player_game_log(self, player, season):
